...
class TestModelCause(common.TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.record = cls.env["mgmtsystem.event.cause"].create({"name": "TestCause"})
        cls.record2 = cls.env["mgmtsystem.event.cause"].create(
            {"name": "test2", "parent_id": cls.record.id}
        )
        cls.record3 = cls.env["mgmtsystem.event.cause"].create(
    ...
    def test_recursion(self):
        parent = self.env["mgmtsystem.event.cause"].create({"name": "ParentCause"})
        child = self.env["mgmtsystem.event.cause"].create(
