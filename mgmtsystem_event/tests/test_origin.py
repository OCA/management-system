...
class TestModelOrigin(common.TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.record = cls.env["mgmtsystem.event.origin"].create({"name": "TestOrigin"})
        cls.record2 = cls.env["mgmtsystem.event.origin"].create(
            {"name": "test2", "parent_id": cls.record.id}
        )
        cls.record3 = cls.env["mgmtsystem.event.origin"].create(
