...
class MgmtsystemNonconformity(models.Model):
    ...
    @api.multi
    def wkf_evaluation(self):
        """Change state to evaluation"""
        return self.write({
        'state': 'evaluation',
        'stage_id': self.env.ref('mgmtsystem_nonconformity.stage_evaluation').id,
        })
    ...
    def action_sign_actions(self):
        ...
        if not self.analysis_date:
            raise exceptions.ValidationError(
                _('Analysis needs to be approved first.')
            )
        self.write({
